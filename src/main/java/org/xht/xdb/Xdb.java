...
import org.xht.xdb.sql.*;
import org.xht.xdb.util.DbTypeUtil;
import org.xht.xdb.util.SpringConnectionUtil;
...
@Slf4j
public class Xdb {
    static final Xdb self = new Xdb();
    private static final XdbSql XDB_SQL = new XdbSql();
     ...
     * @return {@link Xdb}
     */
    public Xdb clearDataSource(String... dataSourceNames) {
     ...
     * @return 返回静态Xdb对象
     */
    public Xdb addDataSource(DataSource dataSource, String dataSourceName) {
        return addDataSource(dataSource, DbTypeUtil.getDbType(dataSource), dataSourceName);
    }

    public Xdb addDataSource(DataSource dataSource) {
        return addDataSource(dataSource, DbTypeUtil.getDbType(dataSource), DEFAULT_DATASOURCE_NAME);
    }
     ...
     * @return 返回静态Xdb对象
     */
    public Xdb addDataSourceDefault(DataSource dataSource) {
        return addDataSourceDefault(dataSource, DbTypeUtil.getDbType(dataSource));
    }
     ...
     * @return 返回静态Xdb对象
     */
    public static XdbSql selectDataSourceDefault() {
    ...
    public static XdbSql datasourceDefault() {
        return selectDataSourceDefault();
    }
     ...
     * @return 返回静态Xdb对象
     */
    public static XdbSql selectDataSourceDefault(String dataSourceName) {
        XDataSource xDataSource = Xdb.self.dataSources.get(dataSourceName);
        if (xDataSource == null) {
            throw new RuntimeException("数据源名称错误，请检查数据源名称是否正确！");
        }
     ...
     * @return 返回静态Xdb对象
     */
    public static XdbSql selectDataSourceByName(String dataSourceName) {
        XDataSource xDataSource = Xdb.self.dataSources.get(dataSourceName);
        Xdb.self.xDataSourceSelected.set(xDataSource);
        return XDB_SQL;
    }
    ...
    /**
     * 选择数据源 同 selectDataSourceByName
     *
     * @param dataSourceName 数据源名称（多数据源时需指定）
     * @return 返回静态Xdb对象
     */
    public static XdbSql datasource(String dataSourceName) {
        return selectDataSourceByName(dataSourceName);
    }
    ...
    public static Connection getConnection() {
        //by xConnection
        Connection connection = getConnectionByXConnection();
        if (connection == null || connection.isClosed() || !connection.isValid(1)) {
            XDataSource xDataSource = Xdb.getXDataSource();
            DataSource dataSource = xDataSource.getDataSource();
            String name = xDataSource.getName();
            //by spring
            if (XdbConfig.isUseSpringTransaction()) {
                connection = SpringConnectionUtil.getConnectionBySpring(dataSource);
            }
            if (connection == null || connection.isClosed() || !connection.isValid(1)) {
                //by datasource
    ...
    private static Connection getConnectionByXConnection() {
        return Xdb.self.xConnection.get();
    }
    ...
    public static SqlTool sqlBuild(SqlBuild sqlBuild) {
        SqlTool sqlTool = getSqlTool();
        sqlTool.sql(sqlBuild.getSql());
        sqlTool.sqlArgs(sqlBuild.getArgs());
        return sqlTool;
    }

    public static SqlTool sqlTool(SqlTool sqlTool) {
        return sqlTool;
    }
     ...
     *                 SqlBuild.init()
     *                         .addWhereStart()
     *                         .addIfNotEmpty("and t2.id=:pid", "pid", id)
     *             )
     *            .sqlOrder("order by t1.id ,t2.id")
     *            .pageIndex(1)
     *            .pagePerSize(10).debug()
     *         ;
     * </pre>
     *
     * @return {@link SqlPageTool }
     */
    public static SqlPageTool sqlPage() {
